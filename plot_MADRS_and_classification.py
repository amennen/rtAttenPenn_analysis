...
projectDir = '/data/jag/cnds/amennen/rtAttenPenn/'
...
for s in np.arange(nsubs):
        if subjects[s] < 100:
                ...
                style = 1
        plt.plot(day_average_diff[s],scores_differences[s,2], marker='.',ms=20,color=colors[style],alpha=0.5)
plt.xlabel('diff in AUC')
plt.ylabel('MADRS change: Post - Pre')
plt.show()
...
for s in np.arange(nsubs):
        if subjects[s] < 100:
                ...
                style = 1
        if s == 0:
            plt.plot(-1*day_average_neg_diff[s],-1*scores_differences[s,0], marker='.',ms=40,color=colors[style],alpha=0.5,label='healthy')
        elif s == 2:
            plt.plot(-1*day_average_neg_diff[s],-1*scores_differences[s,0], marker='.',ms=40,color=colors[style],alpha=0.5,label='depressed')
        plt.plot(-1*day_average_neg_diff[s],-1*scores_differences[s,0], marker='.',ms=40,color=colors[style],alpha=0.5)
