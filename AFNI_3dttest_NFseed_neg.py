...
for s in np.arange(n_HC):
    ...
    subject_stats = "{0}/{1}/{2}/CATEGSEP_seed_correlation_NEG_runAVG.nii.gz".format(noise_save_dir,bids_id,ses_id)
...
command = ("3dttest++ -setA {0} ".format(MDD_subj_str) +
            ...
            "-prefix {0}/{1}/{1}_stats_CATEGSEP_correlation_wb.ttest.nii.gz ".format(second_level,ses_id) +
            "-AminusB " 
            "-prefix_clustsim {0}_stats_CATEGSEP_correlation_wb ".format(ses_id) +
