...
# 			"-prefix {0}/{1}/{1}_stats_{2}_ACC_dlPFC_mask_BLUR0.nii.gz ".format(second_level,ses_id,BRIK_KEY[BRIK]) +
# 			"-AminusB " 
# 			"-covariates {0} ".format(covar_file) +
# 			"-mask {0} ".format(dlPFC_mask) +
# 			"-prefix_clustsim {0}_stats_{1}_ACC_dlPFC_mask_BLUR0 ".format(ses_id,BRIK_KEY[BRIK]) +
#             "-ETAC  " +
# 			"-ETAC_opt NN=2:sid=1:hpow=0:name=BLUR0:pthr=0.01/0.001/10:fpr=9"
# 			)
command = ("3dttest++ -unpooled -setA {0} ".format(MDD_subj_str) +
			"-setB {0} ".format(HC_subj_str) +
			"-prefix {0}/{1}/{1}_stats_{2}_ACC_dlPFC_mask_clustsim_noBLUR.nii.gz ".format(second_level,ses_id,BRIK_KEY[BRIK]) +
			"-AminusB " 
			"-covariates {0} ".format(covar_file) +
			"-mask {0} ".format(dlPFC_mask) +
			"-prefix_clustsim {0}_stats_{1}_ACC_dlPFC_mask_clustsim_noBLUR ".format(ses_id,BRIK_KEY[BRIK]) +
            "-Clustsim  "
			)
print(command)
