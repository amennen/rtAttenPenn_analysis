...
MASK_DIR = '/data/jux/cnds/amennen/rtAttenPenn/MNI_things'
dlPFC_mask = '/data/jux/cnds/amennen/rtAttenPenn/MNI_things/ACC_MFG_IFG_FSL_2mm_bin0p5_resampled.nii.gz'
wholebrain_mask = MASK_DIR + '/' + 'mni_GM_thr0p25_bin_resampled.nii.gz '
...
command = ("3dttest++ -unpooled -setA {0} ".format(MDD_subj_str) +
            ...
            "-prefix {0}/ses-03_minus_ses-01/ses-03_minus_ses-01_stats_{1}_nomask.ttest.nii.gz ".format(second_level,BRIK_KEY[BRIK]) +
            "-AminusB " 
            "-mask {0} ".format(wholebrain_mask) +
            "-covariates {0} ".format(covar_file) +
            "-prefix_clustsim ses-03_minus_ses-01_stats_{0}_nomask ".format(BRIK_KEY[BRIK]) +
